    ...
    stage ('Deploy app on EC2') {
      steps {
        sshagent (credentials: ['sherryinstance']) {
          sh 'ssh -o StrictHostKeyChecking=no ubuntu@34.219.132.183 uptime'
          sh 'ssh -v ubuntu@34.219.132.183 whoami'
          sh 'ssh -v ubuntu@34.219.132.183 aws ecr get-login-password --region us-west-2 | docker login --username AWS --password-stdin 489994096722.dkr.ecr.us-west-2.amazonaws.com'
          sh 'ssh -v ubuntu@34.219.132.183 docker pull 489994096722.dkr.ecr.us-west-2.amazonaws.com/nodeapp:latest'
          sh 'ssh -v ubuntu@34.219.132.183 docker run -d -p 3000 489994096722.dkr.ecr.us-west-2.amazonaws.com/nodeapp:latest'
